...
description: >-
  ...
  labeled by the kind of fun it serves (LeBlanc's eight kinds). Trigger on: "core loop",
...
A concept (title and one-liner, or a brief) and, ideally, the `fun-targeting` result: the 2–3
target kinds of fun and the mechanics. Everything else is optional.
...
6. List the gaps: missing steps, feedback that serves no target kind, a decision with only one
   real option.
7. Write the output in the exact format below.
