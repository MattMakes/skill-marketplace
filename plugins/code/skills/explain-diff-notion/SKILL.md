...
- Use the Notion MCP tools to create a new page and return the URL of the new page. This needs a Notion MCP server configured in the user's own Claude Code settings; the plugin does not wire one. If no Notion tools are available, stop and say so rather than falling back to a file.
