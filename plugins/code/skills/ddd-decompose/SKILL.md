...
Next: /ddd-strategize (classifies each subdomain core/supporting/generic, build vs buy) — or run ddd to continue.
```
...
build-vs-buy and implementation-pattern decision; `ddd-connect`, `ddd-organise` and `ddd-define` then consume the same contexts and relationships. Or run `ddd` to continue the chain automatically.
