...
orchestrated end to end by `ddd`. Tools: Domain Message Flow Modelling (ddd-crew), process-level
...
2. Mode: user said "auto" / "no questions" / "just draft", or you are a subagent or run by `ddd`
...
5. Closing summary a reader can act on: absolute paths written; flows drawn (ids + names); mechanism per
   ...
   open questions; validator result; and the next step: **`/ddd-organise`** (or "run `ddd` to continue").
...
- Run `ddd` to continue the chain or `ddd validate "$DDD" --status` to see where you are.
