...
```bash
node bin/blueprint.mjs deliver <type> <candidate.json> <output.html> --quality showcase --json
```
...
```bash
node bin/blueprint.mjs visual-check <output.html> --json
```
...
```bash
node bin/blueprint.mjs preview <type> <input>.json <output>.html --quality showcase
```
