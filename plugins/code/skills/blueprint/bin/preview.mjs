// blueprint: renamed copy of archify bin/preview.mjs (MIT, tt-a1i)
...
const cliPath = path.join(here, 'blueprint.mjs');
...
<head>
  ...
  <title>Blueprint Live Preview</title>
  <style>
...
<body data-state="checking" data-has-artifact="false">
  <header>
    <span class="brand">Blueprint Preview</span>
  ...
  <main>
    ...
    <iframe id="artifact" title="Verified Blueprint diagram"></iframe>
  </main>
  <script>
    (function () {
      'use strict';
...
export async function startPreview(options) {
  ...
  const stagingDirectory = fs.mkdtempSync(path.join(outputDirectory, '.blueprint-preview-'));
    ...
    state.failure = {
      ...
      message: redactDiagnostic(
        ...
        [
          ...
          [path.resolve(here, '..'), '<blueprint-skill>'],
