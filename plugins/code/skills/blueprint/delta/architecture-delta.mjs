// blueprint: renamed copy of archify delta/architecture-delta.mjs (MIT, tt-a1i)
...
export function compareArchitecture(base, head, evidence = {}) {
  ...
  if (!shared.length) {
    fail('delta/no-shared-component-id', 'The snapshots share no component id, so Blueprint cannot prove that they describe the same system.', {
...
<script id="blueprint-compare-receipt" type="application/json">${safeJson(receipt)}</script>
<script>(()=>{
  ...
  const receiptNode = document.querySelector('#blueprint-compare-receipt');
  ...
  function validateReview() {
    try {
      if (document.querySelectorAll('#blueprint-compare-receipt').length !== 1 || !receiptNode) return false;
  ...
  function recordExport(format, blob, dimensions) {
    document.documentElement.dataset.blueprintDeltaExport = JSON.stringify({
      ok: true,
      format,
      bytes: blob.size,
      mime: blob.type,
  ...
  window.Blueprint.deltaExport = { canonicalSvg: canonicalDeltaSvg, shareCard, exportSvg: exportCanonicalSvg, downloadShareCard };
  window.Blueprint.exportMenu = {
    shareCard,
    run(format) {
...
export function validateArchitectureDeltaHtml(html, receipt) {
  ...
  if ((html.match(/id="blueprint-compare-receipt"/g) || []).length !== 1) failures.push('expected exactly one embedded compare receipt');
  ...
  if (!html.includes('id="export-svg"') || !html.includes('id="share-card"')
    || !html.includes('window.Blueprint.deltaExport = { canonicalSvg: canonicalDeltaSvg, shareCard')) {
