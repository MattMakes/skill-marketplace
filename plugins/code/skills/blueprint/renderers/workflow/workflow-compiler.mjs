// blueprint: renamed copy of archify renderers/workflow/workflow-compiler.mjs (MIT, tt-a1i)
...
function compileWorkflowInternal({
  ...
  try {
    ...
    inputDiagnostics = Array.isArray(error?.blueprintDiagnostics)
      ? error.blueprintDiagnostics.map((diagnostic) => ({
          ...diagnostic,
          supportedFixes: [],
        }))
      : [{
...
function readableAutomaticRoute(edge, from, to, primarySides, primaryPorts) {
  ...
  if (authoredSideFields.length) {
    ...
    const attemptedEvidence = firstFailure?.blueprintDiagnostics?.[0]?.evidence || {};
...
${renderLegend()}
  ...
  try {
    ...
    if (!Array.isArray(error?.blueprintDiagnostics)) throw error;
    const diagnostics = error.blueprintDiagnostics.map((diagnostic) => ({ ...diagnostic }));
