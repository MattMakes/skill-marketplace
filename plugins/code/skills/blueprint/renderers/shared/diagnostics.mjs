// blueprint: renamed copy of archify renderers/shared/diagnostics.mjs (MIT, tt-a1i)
import fs from 'node:fs';
import path from 'node:path';

const DIAGNOSTIC_MODE = process.env.BLUEPRINT_DIAGNOSTIC_FORMAT === 'json';
const recorded = [];
const recordedMessages = new Set();
const boundaryKey = Symbol.for('blueprint.renderer-diagnostic-boundary');
...
function normalizedDiagnostic(diagnostic) {
  const message = String(diagnostic?.message || 'Blueprint could not classify this failure.').trim();
  return {
...
export function throwDiagnosticError(message, diagnostics) {
  ...
  error.blueprintDiagnostics = (diagnostics || []).map(normalizedDiagnostic);
  throw error;
}
...
function rendererFailure(error) {
  const attached = Array.isArray(error?.blueprintDiagnostics)
    ? error.blueprintDiagnostics.map(normalizedDiagnostic)
    : [];
