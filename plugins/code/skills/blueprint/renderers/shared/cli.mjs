// blueprint: renamed copy of archify renderers/shared/cli.mjs (MIT, tt-a1i)
...
export function loadDiagram({ rendererDir, diagramType, defaultExample, argv = process.argv }) {
  ...
  const sourceEvidence = verifyRepositoryEvidence(diagramType, diagram, process.env.BLUEPRINT_REPO_ROOT);
...
export function svgRootAttrs(meta) {
  ...
  const engineeringProfile = meta.engineering_profile
    ...
    : '';
  const requestedProfile = process.env.BLUEPRINT_QUALITY_PROFILE || meta.quality_profile;
  ...
  return `role="img" lang="${esc(resolveLocale(meta.locale))}" aria-labelledby="blueprint-diagram-title blueprint-diagram-description"${animation}${preset}${engineeringProfile} data-quality-profile="${esc(qualityProfile)}"${advisory}`;
}

// Keep the accessible name inside the SVG so it survives standalone SVG
// export and embedding. The fixed IDs are deterministic because an Blueprint
...
export function svgAccessibleText(meta, kind) {
  ...
  return `        <title id="blueprint-diagram-title">${esc(meta.title)}</title>\n        <desc id="blueprint-diagram-description">${esc(description)}</desc>`;
}
...
// canonical SVG is embedded inline outside the full Blueprint viewer.
