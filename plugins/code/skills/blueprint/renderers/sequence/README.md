...
Render `diagram_type: "sequence"` JSON files into the standard Blueprint HTML
template.

```bash
node blueprint/renderers/sequence/render-sequence.mjs input.sequence.json output.html
```

The renderer validates input against `blueprint/schemas/sequence.schema.json`
...
`blueprint/examples/cache-miss-request.sequence.json`.

The schema lives at:

```text
blueprint/schemas/sequence.schema.json
```
