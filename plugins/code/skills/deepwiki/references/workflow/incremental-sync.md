...
| `toc_file` | No | `{output_dir}/toc.json` | Existing TOC |
...
Detects structural differences between `toc.json` and existing documentation files.

**Usage**:
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/deepwiki/scripts/collect_sync_context.py \
  --repo-path "{repo_path}" \
  --toc-file "{toc_file}" \
  --doc-dir "{doc_dir}" \
...
**Usage**:
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/deepwiki/scripts/collect_update_context.py \
  --repo-path "{repo_path}" \
  --toc-file "{toc_file}" \
  --doc-dir "{doc_dir}" \
...
**Usage**:
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/deepwiki/scripts/get_section_update_diff.py \
  --repo-path "{repo_path}" \
