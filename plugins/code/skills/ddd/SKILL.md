...
Invoke `code:ddd` to start a domain design or resume an existing one. You do not need to choose among the nine step skills first.

- **New design:** give a short system brief, the people it serves, and any known constraints. Read the available project context and start with `code:ddd-understand`. If no brief or usable project context exists, ask one focused question about the system and its purpose before creating design artifacts.
- **Existing design:** resolve the existing `ddd/` workspace and show its status, open questions, and next valid step. Resume from that step; do not initialize a second workspace or restart completed work.
- **Orientation only:** if the user just invokes `ddd`, asks where to start, or asks which skill to use, explain the next step and what input it needs. Do not infer a request to run all nine steps.
- **Whole process:** when the user asks for an end-to-end design, run the chain with the mode and checkpoints below. Honor an existing request for auto mode without asking again.

Start the response with the resolved workspace, whether this is a new or resumed design, and the next step. The kickoff coordinates the process; the step skills author the domain model. `code:blueprint` owns the final diagrams, while `code:explain-diff` explains an implementation change later.

Examples: “Use DDD to design a booking system for independent clinics”; “Resume the DDD work in `design/ddd`”; “DDD status”; “Run the strategic steps only.”
...
- The nine `ddd-*` skills installed in `${CLAUDE_PLUGIN_ROOT}/skills/`. Invoke them with the `Skill`
  tool using their namespaced names (`code:ddd-discover`, …) — the bare `ddd-discover`
...
2. Invoke the skill (`Skill` tool → `code:ddd-<step>`), passing the workspace path, mode and depth in
...
deciding alone (`subagent_type: "ddd-decision-strategist"`; `"code:ddd-decision-strategist"` is the
...
(`<ddd-dir>/diagrams/final.architecture.json`, one `final-<flow>.sequence.json` per flow) and renders
`<ddd-dir>/diagrams/final.html` (plus `final-<flow>.html`) through the bundled `code:blueprint` skill.
The renderer lives at `${CLAUDE_PLUGIN_ROOT}/skills/blueprint`; `BLUEPRINT_HOME` can override it
for development. No separate DDD or Blueprint plugin is required. A missing renderer in this
installation is an error; report it rather than claiming a completed render. Name `review.html`, `diagrams/` and `final.html`
(when present) in the coverage summary (`references/coverage-summary.md`).
