...
// usage message when misused. Scripts are the `plugins/code/shared/tests/**/*.mjs` paths on
...
//
// Usage: node plugins/code/shared/tests/checks/root/check-check-scripts.mjs <scope-dir>
...
if (args.length !== 1 || args[0].startsWith("-")) {
  console.error("usage: node plugins/code/shared/tests/checks/root/check-check-scripts.mjs <scope-dir>");
  process.exit(2);
}
...
for (const md of walk(scope)) {
  for (const line of fs.readFileSync(md, "utf8").split("\n")) {
    ...
    for (const tok of m[1].match(/plugins\/code\/shared\/tests\/[^\s"']+\.mjs/g) || []) {
...
if (!scripts.size) { console.error(`usage: no CHECK: lines naming plugins/code/shared/tests/**/*.mjs under ${args[0]}`); process.exit(2); }
