...
//
// Usage: node plugins/code/shared/tests/checks/1.4.1/no-python.mjs <dir>
...
if (args.length !== 1 || args[0].startsWith("-")) {
  console.error("usage: node plugins/code/shared/tests/checks/1.4.1/no-python.mjs <dir>");
  process.exit(2);
}
...
// 2. Scope this historical DDD runtime check to DDD-owned payloads. Other
// code skills legitimately use Python and were never part of this contract.
const roots = fs.existsSync(path.join(dir, 'shared/bin/ddd.mjs'))
  ? ['shared', 'agents/ddd-decision-strategist.md', 'docs/ddd.md',
     ...fs.readdirSync(path.join(dir, 'skills')).filter(n => n === 'ddd' || n.startsWith('ddd-')).map(n => `skills/${n}`)]
  : ['.'];
const outside = [];
function findOutside(abs) {
  for (const entry of fs.readdirSync(abs, { withFileTypes: true })) {
    const file = path.join(abs, entry.name);
    const rel = path.relative(dir, file).split(path.sep).join('/');
    if (roots.some(r => r === '.' || rel === r || rel.startsWith(r + '/'))) continue;
    if (entry.isDirectory() && roots.some(r => r.startsWith(rel + '/'))) findOutside(file);
    else outside.push(rel);
  }
}
findOutside(dir);
const all = scanForPython(dir, { skip: outside });
for (const f of all.pyFiles) failures.push(`python file: ${f}`);
const pycache = [];
(function walk(abs) {
  for (const ent of fs.readdirSync(abs, { withFileTypes: true })) {
    if (!ent.isDirectory() || ent.name === "node_modules" || ent.name === ".git" || outside.includes(path.relative(dir, path.join(abs, ent.name)))) continue;
...
const scoped = scanForPython(dir, { skip: [...outside, ...SKIP] });
