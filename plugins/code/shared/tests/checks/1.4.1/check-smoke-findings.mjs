...
//   (e) modes.md §3b and ddd "Hard decisions" name the plain `"ddd-decision-strategist"`
//       subagent before the namespaced `"code:ddd-decision-strategist"`;
//   (f) shared/README.md has a "Known rough edges" heading.
//
// Usage: node plugins/code/shared/tests/checks/1.4.1/check-smoke-findings.mjs <plugin-dir>
...
if (args.length !== 1 || args[0].startsWith("-")) {
  console.error("usage: node plugins/code/shared/tests/checks/1.4.1/check-smoke-findings.mjs <plugin-dir>");
  process.exit(2);
}
...
const countDocs = [];
for (const skill of fs.readdirSync(skillsDir).filter(n => n === "ddd" || n.startsWith("ddd-")).sort()) {
...
function plainFirst(text, label) {
  const plain = /(?<!code:)"ddd-decision-strategist"/.exec(text);
  const namespaced = /"code:ddd-decision-strategist"/.exec(text);
  if (!plain) failures.push(`(e) ${label}: no quoted plain "ddd-decision-strategist" subagent name`);
  if (!namespaced) failures.push(`(e) ${label}: no quoted "code:ddd-decision-strategist" namespaced form`);
...
if (!fs.existsSync(wf)) failures.push("(e) ddd/SKILL.md missing");
else {
  const sec = /^## Hard decisions[\s\S]*?(?=^## )/m.exec(read(wf));
  if (!sec) failures.push("(e) ddd/SKILL.md: no \"Hard decisions\" section");
  else plainFirst(sec[0], "ddd Hard decisions");
}
