...
//
// Usage: node plugins/code/shared/tests/checks/1.4.1/check-skill-frontmatter.mjs <plugin-dir>
...
if (args.length !== 1 || args[0].startsWith("-")) {
  console.error("usage: node plugins/code/shared/tests/checks/1.4.1/check-skill-frontmatter.mjs <plugin-dir>");
  process.exit(2);
}
...
for (const ent of fs.readdirSync(skillsDir, { withFileTypes: true }).sort((a, b) => (a.name < b.name ? -1 : 1))) {
  if (!ent.isDirectory() || !(ent.name === "ddd" || ent.name.startsWith("ddd-") || ent.name === "blueprint")) continue;
