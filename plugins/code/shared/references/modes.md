...
behave consistently and can be chained by `ddd`.
...
unknown, use the namespaced `"code:ddd-decision-strategist"`, the form a plugin install registers;
...
4. Finish with a short summary a reader who sees only that message can act on: absolute paths written,
   ...
   (`/ddd-<next>` or "run `ddd` to continue").
