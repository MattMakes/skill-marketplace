...
example at `plugins/ddd/shared/examples/mealkit` into a temp directory the caller owns,
so the example is never touched and the mutants cannot drift from it.

```bash
node plugins/ddd/shared/tests/fixtures/build.mjs /tmp/ddd-fixtures   # look at all three by hand
```
