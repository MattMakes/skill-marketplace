...
// can load, and no document anywhere under plugins/ddd or the marketplace manifest names python.
...
// Only the ddd entry is ours; other plugins may legitimately list Python among their languages.
const mk = JSON.parse(readFileSync(path.join(ROOT, ".claude-plugin/marketplace.json"), "utf8"));
const entry = (mk.plugins || []).find((p) => p.name === "ddd");
if (!entry) fails.push("marketplace.json has no ddd entry");
else if (/python/i.test(JSON.stringify(entry))) fails.push("marketplace.json ddd entry mentions python");
if (fails.length) { for (const f of fails) console.error("FAIL " + f); process.exit(1); }
console.log(`docs resolve to real commands; ${n} subcommands dispatchable; no python in plugin docs or marketplace`);
