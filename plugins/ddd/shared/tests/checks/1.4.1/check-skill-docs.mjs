...
// every option, draw the ones that differ in shape, `ddd decision render`), and ddd-workflow
...
//
// Usage: node plugins/ddd/shared/tests/checks/1.4.1/check-skill-docs.mjs <plugin-dir>
...
if (args.length !== 1 || args[0].startsWith("-")) {
  console.error("usage: node plugins/ddd/shared/tests/checks/1.4.1/check-skill-docs.mjs <plugin-dir>");
  process.exit(2);
}
...
if (!fs.existsSync(wf)) failures.push("ddd-workflow/SKILL.md missing");
else {
  checked++;
  for (const p of problems(fs.readFileSync(wf, "utf8"), WORKFLOW_RULES)) failures.push(`ddd-workflow/SKILL.md: ${p}`);
}
...
if (fs.existsSync(wf)) {
  ...
  if (problems(stripped, WORKFLOW_RULES).length < 5) failures.push("control: stripping Step 5 from ddd-workflow did not fail enough rules");
}
...
console.log(`${checked} SKILL.md file(s): PNG rule and decisions[] rule in every step skill, page rebuild and blueprint hand-off in ddd-workflow`);
console.log(MARKER);
