...
```bash
node --test --test-reporter=tap "plugins/ddd/shared/tests/*.test.mjs"
```
...
```bash
node plugins/ddd/shared/tests/golden.mjs                    # replay every command against its snapshot
node plugins/ddd/shared/tests/golden.mjs validate           # one command
node plugins/ddd/shared/tests/golden.mjs --fixture broken-refs contracts check
```
...
is the one that proves no `python3` invocation and no `.py` file is left.
