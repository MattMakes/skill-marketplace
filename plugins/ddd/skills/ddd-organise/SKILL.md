...
- Project knowledge: `CODEOWNERS`, team names in docs/ADRs, `docker-compose`/Kubernetes/serverless files
  (brownfield: today's deployables), what the user says; in a `ddd-workflow` run, the path/mode/depth it passes.
...
2. **Mode** — first match wins: the user said "auto", "no questions", "just draft", "don't ask" →
   ...
   `ddd-workflow` said "mode: auto") → `auto`; `manifest.mode` set → use it; otherwise `interactive`.
...
Next: /ddd-define (bounded context canvases) — or run ddd-workflow to continue
```
...
`ddd-workflow` continues the chain from here. Do not start writing canvases or aggregates yourself.
