#!/usr/bin/env node
// blueprint: renamed copy of archify bin/archify.mjs (MIT, tt-a1i)
...
function usage() {
  return `Usage:
  blueprint render <type> <input.json> [output.html] [--quality standard|showcase] [--repo-root path (architecture only)]
  blueprint compare architecture <base.json> <head.json> [output.html] [--receipt path] [--json] [--quality standard|showcase] [--repo-root path]
  blueprint deliver <type> <input.json> [output.html] [--json] [--open] [--quality standard|showcase] [--repo-root path (architecture only)]
  blueprint preview <type> <input.json> [output.html] [--no-open] [--quality standard|showcase] [--repo-root path (architecture only)]
  blueprint validate <type> <input.json> [--json] [--layout-json] [--quality standard|showcase] [--repo-root path (architecture only)]
  blueprint migrate workflow <old.json> <new.json> --to-schema 2 [--json]
  blueprint inspect <type> <input.json>
  blueprint check <output.html>
  blueprint visual-check <output.html> [--json]
  blueprint guide [scenario or question] [--json] [--lang en|zh]
  blueprint brands [name, alias, domain, or category] [--json]
  blueprint brands capture <url> [--json]
  blueprint examples
  blueprint doctor
  blueprint demo [output-directory]
...
function rendererEnv(quality, repoRoot, diagnosticJson = false) {
  return {
    ...(quality ? { BLUEPRINT_QUALITY_PROFILE: quality } : {}),
    ...(repoRoot ? { BLUEPRINT_REPO_ROOT: repoRoot } : {}),
    ...(diagnosticJson ? { BLUEPRINT_DIAGNOSTIC_FORMAT: 'json' } : {}),
  };
}
...
async function commandCompare(args) {
  ...
  try {
    ...
    reportCompareFailure({ json: options.json, stage: 'prepare', error: 'Architecture compare runtime is unavailable.', code: 'delta/runtime-missing', details: { reason: error.message, supportedFixes: ['install the complete Blueprint skill package'] } });
    return;
  }
  const {
  ...
  try {
    ...
    const outputDiagnostic = error.blueprintDiagnostics?.[0];
  ...
  try {
    stagingDirectory = fs.mkdtempSync(path.join(outputDirectory, '.blueprint-compare-'));
  } catch (error) {
...
function sourceEvidenceFromArtifact(artifact) {
  ...
  const match = html.match(/<script id="blueprint-source-evidence-data" type="application\/json">([\s\S]*?)<\/script>/);
...
async function commandDeliver(args) {
  ...
  try {
    ...
    reportDeliveryFailure({
      ...
      error: error.message,
      diagnostics: error.blueprintDiagnostics || [diagnostic({
  ...
  let stagingDirectory;
  try {
    stagingDirectory = fs.mkdtempSync(path.join(outputDirectory, '.blueprint-delivery-'));
  } catch (error) {
...
async function commandDoctor() {
  ...
  console.error(`\nBlueprint is not ready: ${problems.join('; ')}.`);
  process.exitCode = 1;
}
...
async function commandBrands(args) {
  ...
  if (positional[0] === 'capture') {
    if (positional.length !== 2) fail('Usage: blueprint brands capture <url> [--json]');
  ...
  if (json) {
    console.log(JSON.stringify({
      ...
      count: marks.length,
      marks,
      fallback: 'Run "blueprint brands capture <url> --json", then use the returned digest-pinned brand value.',
    }, null, 2));
    return;
  }
  if (!marks.length) {
    console.log(`No built-in brand matched "${query}". Run "blueprint brands capture <url> --json", then use the returned digest-pinned brand value.`);
    return;
  }
...
function commandDemo(args) {
  ...
  const output = path.join(outputDirectory, 'blueprint-demo.html');
  ...
  console.log('  blueprint render architecture <input.json> <output.html>');
}

function migrationPathDiagnostics(error, sourcePath, destinationPath) {
  if (Array.isArray(error?.blueprintDiagnostics) && error.blueprintDiagnostics.length) {
    return error.blueprintDiagnostics.map((entry) => ({
      ...entry,
...
async function commandMigrate(args) {
  ...
  if (
    ...
    fail('Usage: blueprint migrate workflow <old.json> <new.json> --to-schema 2 [--json]');
  }
  ...
  try {
    ...
    migration = {
      ...
      migrationDiagnostics: [diagnostic({
        ...
        evidence: { reason: error.message },
        supportedFixes: ['report the source workflow and this diagnostic to the Blueprint maintainers'],
      })],
    };
  }
  ...
  try {
    ...
    stagingDirectory = fs.mkdtempSync(path.join(destinationDirectory, '.blueprint-migration-'));
  } catch (error) {
    reportMigrationFailure({
      ...migration,
      migrationDiagnostics: [...migration.migrationDiagnostics, diagnostic({
  ...
  try {
    ...
    const migrationDiagnostics = Array.isArray(error?.blueprintDiagnostics)
...
function commandValidate(args) {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'blueprint-validate-'));
