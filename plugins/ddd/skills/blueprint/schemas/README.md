...
explicit authored viewBox without overlap, Blueprint omits the complete legend
...
by `blueprint brands --json`, or a digest-pinned `{ "url", "sha256" }` object
returned by `blueprint brands capture <url> --json`. Known IDs and known-brand
...
can be produced explicitly with `blueprint migrate workflow ... --to-schema 2`.
...
strictly Blueprint judges composition. `engineering_profile` is an optional
