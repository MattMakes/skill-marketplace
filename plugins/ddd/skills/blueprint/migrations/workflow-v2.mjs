// blueprint: renamed copy of archify migrations/workflow-v2.mjs (MIT, tt-a1i)
...
function schemaDiagnostics(workflow) {
  try {
    ...
    if (Array.isArray(error?.blueprintDiagnostics)) {
      return error.blueprintDiagnostics.map((entry) => ({ ...entry }));
    }
    throw error;
  }
}
  ...
  try {
    ...
    return result({
      ...
      migrationDiagnostics: [diagnostic({
        ...
        supportedFixes: ['report the workflow and compiler receipts to the Blueprint maintainers'],
      })],
      oldRequiredViewBox,
      newRequiredViewBox: requiredViewBoxFrom(planned),
    });
