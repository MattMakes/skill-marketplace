...
Render `diagram_type: "workflow"` JSON files into the standard Blueprint HTML
template.

```bash
node blueprint/renderers/workflow/render-workflow.mjs input.workflow.json output.html
```

The renderer validates input against `blueprint/schemas/workflow.schema.json`
...
```bash
node blueprint/scripts/check-render-output.mjs output.html
```
...
`blueprint/examples/agent-tool-call.workflow.json`; its `schema_version` selects
the applicable contract.

The schema lives at:

```text
blueprint/schemas/workflow.schema.json
```
...
```bash
node blueprint/bin/blueprint.mjs migrate workflow old.json new.json --to-schema 2 --json
```
...
```bash
node blueprint/bin/blueprint.mjs validate workflow input.workflow.json --layout-json
```
