// blueprint: renamed copy of archify renderers/shared/brand-marks.mjs (MIT, tt-a1i)
...
const RESOLVED_MARK = Symbol('blueprint.brandMark');
const MAX_HTML_BYTES = 256 * 1024;
const MAX_IMAGE_BYTES = 1024 * 1024;
const MAX_CAPTURE_CONCURRENCY = 3;
const DEFAULT_CAPTURE_TIMEOUT_MS = 8000;
const USER_AGENT = 'Blueprint/2.17-blueprint.1 brand-preview';
...
function validateUrlShape(url, allowPrivate = process.env.BLUEPRINT_BRAND_ALLOW_PRIVATE === '1') {
...
async function resolveRequestTarget(url, deadline) {
  const allowPrivate = process.env.BLUEPRINT_BRAND_ALLOW_PRIVATE === '1';
...
function captureTimeoutMilliseconds() {
  const configured = Number(process.env.BLUEPRINT_BRAND_CAPTURE_TIMEOUT_MS);
...
export async function prepareDiagramBrandMarks(diagramType, diagram) {
  ...
  await mapConcurrent(nodes, MAX_CAPTURE_CONCURRENCY, async (node, index) => {
    ...
    if (url) {
      unknown.push(`/${collection}/${index}/brand ${JSON.stringify(node.brand)} is an unpinned URL; capture it first with \`blueprint brands capture ${url.href} --json\``);
      return;
    }
  ...
  if (unknown.length) {
    throwDiagnosticError(`Brand mark validation failed:\n- ${unknown.join('\n- ')}`, unknown.map((message) => ({
      ...
      supportedFixes: message.includes('is an unpinned URL')
        ? ['run `blueprint brands capture <url> --json` and author the returned digest-pinned brand object']
        : ['choose an ID from `blueprint brands`', 'run `blueprint brands capture <url> --json` for an unknown official site'],
    })));
  }
}
