// blueprint: renamed copy of archify renderers/shared/utils.mjs (MIT, tt-a1i)
import {
  escapeHtml as esc,
  localizeTemplate,
  resolveLocale,
...
const SVG_SLOT_RE = /      <!-- BLUEPRINT:SVG_SLOT_START -->[\s\S]*?      <!-- BLUEPRINT:SVG_SLOT_END -->/;
const CARDS_SLOT_RE = /    <!-- BLUEPRINT:CARDS_SLOT_START -->[\s\S]*?    <!-- BLUEPRINT:CARDS_SLOT_END -->/;
const SUBTITLE_SLOT_RE = /^([ \t]*)<p class="subtitle">\[Subtitle description\]<\/p>[ \t]*(\r?\n)?/m;
const GUIDED_VIEWS_PLACEHOLDER = '<!-- BLUEPRINT:GUIDED_VIEWS_DATA -->';
const SOURCE_EVIDENCE_PLACEHOLDER = '    <!-- BLUEPRINT:SOURCE_EVIDENCE_DATA -->';
const I18N_PLACEHOLDER = '    <!-- BLUEPRINT:I18N_DATA -->';
...
export function applyTemplate(template, {
  ...
  if (!SVG_SLOT_RE.test(template)) {
    throw new Error('applyTemplate: template missing BLUEPRINT:SVG_SLOT sentinel');
  }
  if (!CARDS_SLOT_RE.test(template)) {
    throw new Error('applyTemplate: template missing BLUEPRINT:CARDS_SLOT sentinel');
  }
  ...
  const renderedSubtitle = typeof subtitle === 'string' && subtitle.trim()
    ...
    : '';
  const i18nData = `    <script id="blueprint-i18n-data" type="application/json">${i18nJson}</script>`;
  ...
  return templateWithI18n
    ...
    .replace(GUIDED_VIEWS_PLACEHOLDER, () => `<script id="blueprint-guided-views-data" type="application/json">${guidedViewsJson}</script>`)
    .replace(SOURCE_EVIDENCE_PLACEHOLDER, () => sourceEvidence
      ? `    <script id="blueprint-source-evidence-data" type="application/json">${sourceEvidenceJson}</script>`
      : '');
}
