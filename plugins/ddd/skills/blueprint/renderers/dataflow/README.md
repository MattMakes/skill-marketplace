...
Render `diagram_type: "dataflow"` JSON files into the standard Blueprint HTML
template.

```bash
node blueprint/renderers/dataflow/render-dataflow.mjs input.dataflow.json output.html
```

The renderer validates input against `blueprint/schemas/dataflow.schema.json`
...
`blueprint/examples/product-analytics.dataflow.json`.

The schema lives at:

```text
blueprint/schemas/dataflow.schema.json
```
...
visual-only because Blueprint has no compiled edge-kind facts in this slice. A
