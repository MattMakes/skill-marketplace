...
description: Step 2 of the DDD Starter Modelling Process chain (ddd-understand → ddd-discover →
  ...
  questions, assumptions logged). Run ddd-workflow for the whole process.
---
...
step 2 of the nine-step `ddd-*` chain (`ddd-workflow` runs all of them). `ddd-decompose` will cut
...
`/ddd-decompose`, or "run `ddd-workflow` to continue".
...
the ids you minted are now load-bearing, then hand off — or say "run `ddd-workflow` to continue".
